...
def plot_exp():
    ...
    # ax: Axes = widget.ax
    widget.ax.set(xlabel="Time (s)", ylabel="Counts", yscale="log", title=title)
    ...
    for counter, key in enumerate(data.keys()):
        ...
        if not key == "SUM":
            ...
            _expPlot = widget.ax.errorbar(time, counts, error, fmt=".", ms=7, label=key, c=pltColors[counter], alpha=1)
            expPlot.append(_expPlot)

    exp_sum_data = get_time_and_counts_and_error(data["SUM"])
    exp_sum_plot = widget.ax.errorbar(
    ...
    for counter, fitted_data in enumerate(dNdtSol):
        (_fitPlot,) = widget.ax.plot(simulateTime, fitted_data, "-", c=pltColors[counter], alpha=1, animated=True)
        fitPlot.append(_fitPlot)

    fitted_sum = dNdtSol.sum(axis=0)
    (fitted_sum_plot,) = widget.ax.plot(simulateTime, fitted_sum, "-", c="k", alpha=1, animated=True)
    fitPlot.append(fitted_sum_plot)

    widget.blit = BlitManager(widget.canvas, fitPlot)
    legend = widget.ax.legend([*legends, "SUM"], ncol=round(len(legends) / 3))
    ...
    # widget.ax.set_ylim(ymin=0.1)
    # widget.ax.set_xlim(xmin=-0.1)
