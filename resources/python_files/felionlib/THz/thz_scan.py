...
def plot_thz():
    ...
    for i, filename in enumerate(filenames):
        ...
        dataToSend["thz"]["individual"][f"{filename.name}"] = {
            "x": list(freq),
            "y": list(depletion_counts),
            "name": lg,
            "mode": "lines+markers",
            "type": "scattergl",
            "fill": "tozeroy",
            "line": {"color": f"rgb{colors[i*2]}", "shape": "hvh"},
        }

        dataToSend["resOnOff_Counts"][f"{filename.name}_On"] = {
            "x": list(freq),
            "y": resOnCounts.mean(axis=0).tolist(),
            "name": f"{filename.name}_On",
            "mode": "markers",
            "type": "scattergl",
            "line": {"color": f"rgb{colors[i*2]}", "shape": "hvh"},
        }
        ...
        dataToSend["resOnOff_Counts"][f"{filename.name}_Off"] = {
            "x": list(freq),
            "y": resOffCounts.mean(axis=0).tolist(),
            "name": f"Off: {freq_resOff}GHz: {iteraton}",
            "mode": "markers",
            "type": "scattergl",
        ...
        fittedParamsTable, fittedInfos = fitData(
            freq, depletion_counts, varyAll=varyAll, method=fitMethod, paramsTable=paramsTable, fitfile=fitfile
        )
    ...
    fittedParamsTable, fittedInfos = fitData(
        binx, biny, varyAll=varyAll, fitfile=fitfile, method=fitMethod, paramsTable=paramsTable
    )
