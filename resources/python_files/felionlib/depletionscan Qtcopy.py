...
class depletionplot:
    ...
    def __init__(
        ...
        try:
            ...
            qapp = QApplication([])
            self.widget = felionQtWindow(title="Depletion Plot", location=self.location, createControlLayout=False)
            self.create_figure()
    ...
    def create_figure(self):
        ...
        # self.widget.fig, self.widget.canvas = self.widget.Figure(default_widget=False, default_save_widget=False, executeCodeWidget=False, dpi=200)
        # self.widget.fig.suptitle("Depletion Scan")
        # self.ax0, self.ax0 = self.widget.fig.subplots(1, 2)
        self.widget.fig.suptitle("Depletion Scan")
        self.widget.fig.subplots_adjust(top=0.86, bottom=0.20, right=0.97, wspace=0.34)
        self.ax0 = self.widget.fig.add_subplot(121)
        self.ax1 = self.widget.fig.add_subplot(122)

        self.widget.createControlLayout(axes=(self.ax0, self.ax0))

    # def change_title(self, event=None): 
    #     self.widget.fig.suptitle(self.widget.plotTitle.get())
    ...
    def depletion_widgets(self, Koff, Kon, N, Na0, Nn0):
        self.slider_value_lists = [np.linspace(0, val, 100) for val in [Koff, Kon, N, Na0, Nn0]]
        slider_layout_vbox =  QtWidgets.QVBoxLayout()

        slider_layout_koff, self.koff_slider = self.widget.makeSlider("Koff", callback=self.update)
        slider_layout_N, self.n_slider = self.widget.makeSlider("N", callback=self.update)
        slider_layout_kon, self.kon_slider = self.widget.makeSlider("Kon", callback=self.update)
        slider_layout_na, self.na_slider = self.widget.makeSlider("Na", callback=self.update)
        slider_layout_nn, self.nn_slider = self.widget.makeSlider("Nn", callback=self.update)
    ...
    def update(self, event=None):
        ...
        Koff = self.slider_value_lists[0][self.koff_slider.value()]
        Kon = self.slider_value_lists[1][self.kon_slider.value()]
        N = self.slider_value_lists[2][self.n_slider.value()]
        Na0 = self.slider_value_lists[3][self.na_slider.value()]
        Nn0 = self.slider_value_lists[4][self.nn_slider.value()]
    ...
    #         self.widget.fig.savefig(save_file, dpi=self.widget.dpi_value.get())
