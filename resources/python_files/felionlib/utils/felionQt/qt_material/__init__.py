...
import jinja2
TEMPLATE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "material.css.template")
...
# ----------------------------------------------------------------------
def build_stylesheet(theme="", invert_secondary=False, extra={}, parent="theme", template=None, export=False):
    """"""
    ...
    if GUI:
        ...
        color = QColor(*[int(theme["primaryColor"][i : i + 2], 16) for i in range(1, 6, 2)] + [92])
...
def get_theme(theme_name, invert_secondary=False):
    if theme_name in ["default.xml", "default_dark.xml", "default", "default_dark"]:
        theme = os.path.join(os.path.dirname(os.path.abspath(__file__)), "themes", "dark_teal.xml")
    elif theme_name in ["default_light.xml", "default_light"]:
        invert_secondary = True
        theme = os.path.join(os.path.dirname(os.path.abspath(__file__)), "themes", "light_blue.xml")
    elif not os.path.exists(theme_name):
        theme = os.path.join(os.path.dirname(os.path.abspath(__file__)), "themes", theme_name)
    else:
        theme = theme_name
    ...
    theme = {child.getAttribute("name"): child.firstChild.nodeValue for child in document.getElementsByTagName("color")}

    for k in theme:
        os.environ[str(k)] = theme[k]

    if invert_secondary:
        theme["secondaryColor"], theme["secondaryLightColor"], theme["secondaryDarkColor"] = (
    ...
    for font_dir in ["roboto"]:
        for font in filter(lambda s: s.endswith(".ttf"), os.listdir(os.path.join(fonts_path, font_dir))):
            try:
                ...
                QFontDatabase.add_application_font(os.path.join(fonts_path, font_dir, font))


# ----------------------------------------------------------------------
def apply_stylesheet(app, theme="", style=None, save_as=None, invert_secondary=False, extra={}, parent="theme", template=None):
    """"""
    if style:
        try:
            try:
    ...
    stylesheet = build_stylesheet(theme, invert_secondary, extra, parent, template=template)
...
def set_icons_theme(theme, parent="theme"):
    ...
    if GUI:
        try:
            ...
            QDir.add_search_path("qt_material", os.path.join(os.path.dirname(__file__), "resources"))
...
def deprecated(replace):
    ...
    def wrap1(fn):
        ...
        def wrap2(*args, **kwargs):
            logging.warning(f'This function is deprecated, please use "{replace}" instead.')
            fn(*args, **kwargs)
...
class QtStyleTools:
    ...
    def apply_stylesheet(self, parent, theme, invert_secondary=False, extra={}, callable_=None, template=None):
        """"""
        if theme == "default":
        ...
        apply_stylesheet(parent, theme=theme, invert_secondary=invert_secondary, extra=extra, template=template)
        if callable_:
            callable_()
    ...
    def update_theme_event(self, parent):
        """"""
        try:
            density = [action.text() for action in self.menu_density_.actions() if action.isChecked()][0]
            theme = [action.text() for action in self.menu_theme_.actions() if action.isChecked()][0]
        except:
            density = [action.text for action in self.menu_density_.actions() if action.checked][0]
            theme = [action.text for action in self.menu_theme_.actions() if action.checked][0]
    ...
    def update_theme(self, parent):
        ...
        self.apply_stylesheet(
            parent, "my_theme.xml", invert_secondary=light, extra=self.extra_values, callable_=self.update_buttons
        )
    ...
    def show_dock_theme(self, parent):
        ...
        try:
            ...
            parent.add_dock_widget(Qt.DockWidgetArea.LeftDockWidgetArea, self.dock_theme)
