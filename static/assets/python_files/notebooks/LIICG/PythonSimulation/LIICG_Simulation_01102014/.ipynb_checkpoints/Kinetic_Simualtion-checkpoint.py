...
def calculate_einstein_coefficients(R):
    nu = 453e9  # transition frequency in Hz
    R.A_10 = 6.24e-4  # from CDMS
    R.B_10 = c**3*R.A_10/8/pi/h/nu**3
    R.B_01 = 3*R.B_10

    print(f"{R.B_10=}, {R.B_10=}")
...
def calculate_occupancy(R, CD, I):
    ...
    R.q_10 = 1*4.3242e-11  # value from Turpin et al. for CH+; linear approx
    R.q_01 = R.q_10*g[1]/g[0]*np.exp(-Energy[1]/0.695035/CD.temp)/np.exp(-Energy[0]/0.695035/CD.temp)  # calculating q_up from q_down detailed balancing
    R.q_20 = 1*3.4640e-11  # value from Turpin et al. for CH+; linear approx
    R.q_02 = R.q_20*g[2]/g[0]*np.exp(-Energy[2]/0.695035/CD.temp)/np.exp(-Energy[0]/0.695035/CD.temp)  # calculating q_up from q_down detailed balancing
    R.q_21 = 1*1.3013e-10  # value from Turpin et al. for CH+; linear approx
    R.q_12 = R.q_21*g[2]/g[1]*np.exp(-Energy[2]/0.695035/CD.temp)/np.exp(-Energy[1]/0.695035/CD.temp)  # calculating q_up from q_down detailed balancing
    #print(R.q_01)
    #print(R.q_02)
    #print(R.q_12)
...
def calculate_rates(conditions, CD, RA, R, T, c, pi):
    sigma = 453521850000*np.sqrt(8*conditions[6]*k_boltzmann*np.log(2)/c**2/conditions[7]/m)
    gamma = 4.9e7*np.sqrt(conditions[0])

    z = 1j*gamma/(sigma*np.sqrt(2))

    norm = (special.wofz(z)/(sigma*np.sqrt(2*np.pi))).real
    
    print(f"{sigma=}, {gamma=}, {norm=}, {CD.P_THz/CD.A_trap/c*norm}" )
    ...
    print(f"{RA.Rate_q_01/T.timestep:.2e}, {RA.Rate_q_10/T.timestep:.2e}")
    print(f"{RA.Rate_q_02/T.timestep:.2e}, {RA.Rate_q_20/T.timestep:.2e}")
    print(f"{RA.Rate_q_12/T.timestep:.2e}, {RA.Rate_q_21/T.timestep:.2e}")
...
def simulation(conditions, RA, R, CD, I, T, j):
    ...
    #     file = open("D:\Daten\Dokumente\SimDaten\LevelPop\LevelPopulation_on_THzPower=" + str(CD.P_THz) +"_a=" + str(a) + "_n_He=" + str(CD.n_He) + "_TrapTime=" + str(CD.time) + "_Temp=" + str(CD.temp) + ".txt", "w")
    ...
    #     file.close()
    #     file = open("D:\Daten\Dokumente\SimDaten\LevelPop\LevelPopulation_off_THzPower=" + str(CD.P_THz) +"_a=" + str(a) + "_n_He=" + str(CD.n_He) + "_TrapTime=" + str(CD.time) + "_Temp=" + str(CD.temp) + ".txt", "w")
