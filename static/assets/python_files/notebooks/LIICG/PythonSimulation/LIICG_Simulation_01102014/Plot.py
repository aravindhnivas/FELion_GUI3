...
def plot_LevelPop():
    ...
    y5 = []
    y6 = []
    file1 = open("D:\Daten\Dokumente\SimDaten\LevelPop\LevelPopulation_off_THzPower=2e-05_a=0.5_n_He=500000000000000.0_TrapTime=2000_Temp=5.7.txt", "r")
    length = 400000
    ...
    pos_y4 = y4[x_max-1]
    pos_y5 = y5[x_max-1]
    py.plot(y1, color="#FF0000", label="CD$^+$(0)")
    py.plot(y2, color="#83AF23", label="CD$^+$(1)")
    py.plot(y3, color="#0082C6", label="CD$^+$(2)")
    py.plot(y4, color="#590F68", label="HeCD$^+$")
    py.plot(y5, color="#FF7F00", label="He$_2$CD$^+$")
    py.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3, ncol=5, mode="expand", borderaxespad=0., fontsize=13)
    py.xlim(0, x_max)
    py.ylim(0, y_max)
    x_ticks = [0]
    x_pos = [0]
    for i in range(5):
        ...
        x_ticks.append(x_value*(i+1)*0.005)
    py.xticks(x_pos, x_ticks)
    py.xlabel("time [ms]", fontdict=F.font2)
    py.ylabel("amount of ions", fontdict=F.font2)
    path = "D:\Daten\Dokumente\SimDaten\_NewPlots\LIICG_Sim_TrapTime_LevelPop3.png"
