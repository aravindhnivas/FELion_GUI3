<script>
    ...
    async function readCollisionalDataFromFile() {
        ...
        rateConstantsRawData.forEach((currentRate) => {
            currentRate = currentRate.trim().split("\t").map(r => r.trim())
    ...
    const fitDataFunction = () => {
        ...
        const fitData = dataToPlot.map(dataObj=>{
            ...
            const {equation} = polynomial(tranposedArr, { order: parseInt(polyOrder) });

            
            const currentCollisionalRateConstant = equation.map((coeff, index)=>{
                const powerOrder = polyOrder - index
                return coeff*collisionalTemp**powerOrder
            }).reduce((a, b)=>a+b)

            console.log({currentCollisionalRateConstant}, currentCollisionalRateConstant/parseFloat(scaleRateConstant))
            ...
            const fittedRateConstant = fittedTemp.map(x=>{
                return equation.map((coeff, index)=>{
                    const powerOrder = polyOrder - index
...
>
    <svelte:fragment slot="header_content__slot" >
        ...
        <div class="header">
            ...
            <Textfield bind:value={scaleRateConstant} label="ScaleY" 
