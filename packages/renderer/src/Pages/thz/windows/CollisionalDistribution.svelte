<script lang="ts">
    ...
    async function computeCollisionalProcess(e?: Event) {
        ...
        try {
            ...
            const { 
                data, collisionalBoltzmanPlotData, differenceFromBoltzman 
            } = <{ [key: string]: DataFromPython }>(
                dataFromPython
            )
            ...
            save_plot_datas.boltzman_comparision.collisionalData = {
                ...
                plot_kwargs: {
                    ...
                    ls: 'None', marker:'o', ms: '5', color: 'black'
                }
    ...
    const figXlabel = persistentWritable('collisionalDistribution_figXlabel', 'J')
    let figYlabel = 'Population ratio'
...
<SeparateWindow {title} bind:active bind:windowReady bind:graphWindow maximize={false}>
    ...
    <svelte:fragment slot="header_content__slot">
        <div class="align no-wrap" on:keyup={({code}) => {
            if (code === 'Enter') {
