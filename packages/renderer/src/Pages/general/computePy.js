...
export default async function ({
    ...
    try {
        if (!get(pyServerReady)) {
            ...
            if (!serverSpawned) {
                window.handleError(
                    'Python server is not ready. Fix it in Settings --> Configuration'
                )
        ...
        if (general) {
            ...
            if (target) {
                ...
                if (processDivGeneral.length === 0) {
                    ...
                    processDivGeneral.className =
                        'tag__span tag is-warning hide'
                    ...
                    processDivGeneralNum = isNaN(parseInt(num))
                        ? 0
                        : parseInt(num)
                    processDivGeneral.textContent = `${
                        processDivGeneralNum + 1
                    }`
                }
