...
<script>
    ...
    function computeParameters() {
        ...
        if(currentData) {
            ...
            totalMass.forEach(m=>{modifiedTotalMass[m]={included:true}})
    ...
    async function kineticSimulation(e) {
        try {
            let error = false;
            if(!selectedFile) {createToast("Select a file", "danger"); error=true}
            if(!currentData) {createToast("No data available", "danger"); error=true}
            if(!fs.existsSync(kineticfile)) {createToast("Compute and save kinetic equation", "danger"); error=true}
            
            if(error) {
                const submitbtn = document.getElementById("kinetic-submit-button")
    ...
    let kineticEditorLocation = db.get(`${kineticEditorFiletype}-report-md`) || currentLocation
    ...
    onMount(()=>{
        loadConfig()
        if(fileCollections.length>0) {selectedFile=fileCollections[0]}
    })
    ...
    const update_included_masses = ({node, mass}) => {
        ...
        for(const mass in modifiedTotalMass) {
            if(modifiedTotalMass[mass].included) {newTotalMass = [...newTotalMass, mass]}
        }
    ...
    <svelte:fragment slot="main_content__slot">
        ...
        <div class="main_container__div">
            <div class="align box h-center">
                ...
                <Textfield value={numberDensity} label="numberDensity" disabled />
                ...
                <div class="align h-center">
                    ...
                    {#each totalMass as mass (mass)}
                        <span class="tag is-warning">
                            ...
                            {mass}
                            <button class="delete is-small" on:click={({target})=>{update_included_masses({node: target?.parentNode, mass})}}></button>
                        </span>
                    {/each}
