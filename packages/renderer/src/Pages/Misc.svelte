<script>
    ...
    $: kelvin = Number((plank_constant / boltzman_constant) * hz).toFixed(
        fixedDigit
    )
    $: cm_1 = Number(hz / (c * 1e2)).toFixed(fixedDigit)
    $: um = Number((c / hz) * 1e6).toFixed(fixedDigit)
    $: ghz = Number(hz * 1e-9).toFixed(fixedDigit)
    $: nm = Number((c / hz) * 1e9).toFixed(fixedDigit)
            ...
            <div class="box">
                ...
                <div class="unit_conversion_contents">
                    <div class="align">
                        ...
                        <Textfield
                            bind:value={um}
                            on:change={() => (hz = (c / um) * 1e6)}
                            label="Micron"
                        />
                        <Textfield
                            bind:value={cm_1}
                            on:change={() => (hz = cm_1 * c * 1e2)}
                            label="cm-1"
                        />
                        <Textfield
                            bind:value={kelvin}
                            on:change={() =>
                                (hz =
                                    (boltzman_constant / plank_constant) *
                                    kelvin)}
                            label="Kelvin"
                        />
                        <Textfield
                            bind:value={eV}
                            on:change={() =>
                                (hz = (electron_charge / plank_constant) * eV)}
                            label="eV"
                        />
                        <Textfield
                            bind:value={J}
                            on:change={() => (hz = J / plank_constant)}
                            label="Joule"
                        />

                        <Textfield
                            bind:value={nm}
                            on:change={() => (hz = (c / nm) * 1e9)}
                            label="nm"
                        />
                    </div>
                    ...
                    />
                    <div class="align">
                        <div class="subtitle is-pulled-left">
                            Fundamental constants
                        </div>

                        <CustomSwitch
                            style="margin: 0 1em;"
                            bind:selected={edit_constants}
                            label="Edit"
                        />
                    </div>
                    <div class="align">
                        <Textfield
                            disabled={!edit_constants}
                            bind:value={c}
                            label="Speed of light in vaccum (cm/s)"
                        />
                        <Textfield
                            disabled={!edit_constants}
                            bind:value={boltzman_constant}
                            label="Boltzman constant (J/K)"
                        />
                        <Textfield
                            disabled={!edit_constants}
                            bind:value={plank_constant}
                            label="Plank's constant (J.s)"
                        />
                        <Textfield
                            disabled={!edit_constants}
                            bind:value={electron_charge}
            ...
            <div class="box">
                ...
                <div class="unit_conversion_contents">
                    ...
                    <Textfield
                        ...
                        bind:value={pq1_before}
                        label="Before letting in gas"
                    />
                    <Textfield
                        style="width:90%; margin-right:0.5em;"
                        bind:value={pq1_after}
                    ...
                    <Textfield
                        style="width:90%; margin-right:0.5em;"
                        disabled={true}
                        value={ndensity.toExponential(4)}
                        label="Number density"
                    />
                    <hr />
                    <CustomSwitch
                        style="margin: 0 1em;"
                        bind:selected={edit_numberDensity_constants}
                        label="Edit"
                    />

                    <Textfield
                        style="width:90%; margin-right:0.5em;"
                        disabled={!edit_numberDensity_constants}
