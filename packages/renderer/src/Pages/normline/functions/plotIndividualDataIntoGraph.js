...
export const get_data = (data) => {
    let dataPlot = [];
    for (const x in data) { dataPlot.push(data[x]) }
    return dataPlot
}
    ...
    if(plotfile !== "average") {
        ...
        if(get(opoMode)) {
            ...
            if(data["SA"]?.["Calibration"]) {
                ...
                plot(
                    "OPO Calibration", 
                    "Set Wavenumber (cm-1)", "Measured Wavenumber (cm-1)", SA, "opoSA"
                    )
            }
        } else {
            ...
            subplot(
                ...
                "Wavelength set (cm-1)", "SA (cm-1)", SA, "saPlot",
                "Wavelength (cm-1)", `Total Power (mJ)`, pow
            );
        }
    }
    const dataToPlot = {}
