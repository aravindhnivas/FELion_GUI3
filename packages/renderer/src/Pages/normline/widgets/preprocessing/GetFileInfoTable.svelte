...
<div class="align">
    ...
    {#if toggleFileDetailsTable}
        <Table 
            id="felix_filedetails_table" bind:rows={$filedetails}
            closeOption={false} sortOption={true} animateRow={false}
            
            head={[
                "Filename", "min(cm-1)", "max(cm-1)", "Trap(s)", 
                "B0(ms)", "Res.(V)", "IE(eV)", "Temp(K)", "Precursor"
            ]}
