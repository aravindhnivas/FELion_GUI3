...
export function plot(mainTitle, xtitle, ytitle, data, graphDiv, logScale=null, createPlotlyClickEvent=false) {
    ...
    try {
        ...
        if(!get(plotlyEventsInfo)[graphDiv]) {get(plotlyEventsInfo)[graphDiv] = {}}
    ...
    catch (err) { console.error("Error occured while plotting\n", err); window.handleError(err); }
}
...
export function plotlyClick(graphDiv) {
    ...
    graph.on('plotly_click', data => {
        if(data.event.ctrlKey) {
            ...
            relayout(graph ,{annotations: get(plotlyEventsInfo)[graphDiv].annotations})
            console.log({graph, annotations: get(plotlyEventsInfo)[graphDiv].annotations})
        }
    })
